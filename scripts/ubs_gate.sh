...
if command -v mktemp >/dev/null 2>&1; then
  report_tmp=$(mktemp "${TMPDIR:-/tmp}/assess-ubs-gate-report.XXXXXX")
else
  report_tmp="${TMPDIR:-/tmp}/assess-ubs-gate-$$.json"
  : >"$report_tmp"
fi
if command -v mktemp >/dev/null 2>&1; then
  log_file=$(mktemp "${TMPDIR:-/tmp}/assess-ubs-gate-log.XXXXXX")
else
  log_file="${TMPDIR:-/tmp}/assess-ubs-gate-$$.log"
  : >"$log_file"
fi
ignore_file_tmp=""
if [ "$#" -eq 1 ] && [ "$1" = "." ]; then
  if command -v mktemp >/dev/null 2>&1; then
    ignore_file_tmp=$(mktemp "${TMPDIR:-/tmp}/assess-ubs-gate-ignore.XXXXXX")
  else
    ignore_file_tmp="${TMPDIR:-/tmp}/assess-ubs-gate-ignore-$$.txt"
    : >"$ignore_file_tmp"
  fi
...
set +e
"$ubs_bin" --ci --only=rust $ubs_ignore_args "$@" --report-json "$report_tmp" >"$log_file" 2>&1
ubs_exit=$?
set -e
